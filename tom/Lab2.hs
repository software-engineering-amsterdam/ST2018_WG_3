...
ass31, ass32, ass33, ass34 :: Int -> Bool
ass31 = (\ x -> even x && x > 3)
ass32 = (\ x -> even x || x > 3)
ass33 = (\ x -> (even x && x > 3) || even x)
ass34 = (\ x -> (even x && x > 3) || even x)

ass3DescStrength :: [(Int -> Bool)]
ass3DescStrength = sortBy (ordStronger [(-10)..10]) [ass31, ass32, ass33, ass34, even]
    where ordStronger :: [a] -> (a -> Bool) -> (a -> Bool) -> Ordering
          ordStronger xs p q = if stronger xs p q then LT else GT
          

{-- Assignment 4 --}

isPermutation :: Eq a => [a] -> [a] -> Bool
isPermutation as bs = bs `elem` permutations as

testIsPermutation :: Bool
testIsPermutation = isPermutation [1, 2, 4, 10] [2, 10, 4, 1]
                 && isPermutation [5, 4, 3, 8] [8, 4, 3, 5]
                 && not (isPermutation [5, 4, 3, 8] [8, 3, 4, 5, 1])
                 && not (isPermutation [1, 4, 2] [])
                 
autoTestIsPermutation = undefined {-- The test would be the same as the function itself? --}
